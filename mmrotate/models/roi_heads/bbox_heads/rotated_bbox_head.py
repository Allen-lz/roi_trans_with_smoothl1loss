...
class RotatedBBoxHead(BaseModule):
    ...
    def _get_target_single(self, pos_bboxes, neg_bboxes, pos_gt_bboxes,
        ...
        iou_bbox_targets = pos_bboxes.new_zeros(num_samples, 5)
        if num_pos > 0:
            labels[:num_pos] = pos_gt_labels
            pos_weight = 1.0 if cfg.pos_weight <= 0 else cfg.pos_weight
            label_weights[:num_pos] = pos_weight
            if not self.reg_decoded_bbox:
                # 如果不是使用decoder就使用encoder
                # 如果是decoded模式的话, target就不采用offset的模式
                # 但是对pred_bbbox依旧要用decoder，因为即使是使用的是iou，还是希望model最后输出的还是offset的模式
                # 增加一项iou_pos_bbox_targets = pos_gt_bboxes, 其余的不变
                pos_bbox_targets = self.bbox_coder.encode(
                    pos_bboxes, pos_gt_bboxes)
                # iou_pos_bbox_targets变量只要跟着pos_bbox_targets的足迹就行了
                iou_pos_bbox_targets = pos_gt_bboxes
            else:
                ...
                # 这行加了只是为了代码好看而已
                iou_pos_bbox_targets = self.bbox_coder.encode(
                    pos_bboxes, pos_gt_bboxes)

            bbox_targets[:num_pos, :] = pos_bbox_targets
            bbox_weights[:num_pos, :] = 1
            iou_bbox_targets[:num_pos, :] = iou_pos_bbox_targets

        if num_neg > 0:
            label_weights[-num_neg:] = 1.0

        return labels, label_weights, bbox_targets, bbox_weights, iou_bbox_targets
    ...
    def get_targets(self,
        ...
        labels, label_weights, bbox_targets, bbox_weights, iou_bbox_targets = multi_apply(
        ...
        if concat:
            ...
            iou_bbox_targets = torch.cat(iou_bbox_targets, 0)
        ...
        if bbox_pred is not None:
            ...
            if pos_inds.any():
                if self.reg_decoded_bbox:
                    # 这个好, 作者自己就考虑到了这一点
                    # 但是这里回归用的还是offset, 只是在计算回归loss的时候会用到IoU来计算模
                    # 所以self.reg_decoded_bbox这里是不开的, 只是会使用到下面的bbox_pred = self.bbox_coder.decode(rois[:, 1:], bbox_pred)
                    # When the regression loss (e.g. `IouLoss`,
                    # `GIouLoss`, `DIouLoss`) is applied directly on
                    # the decoded bounding boxes, it decodes the
                    # already encoded coordinates to absolute format.
                    iou_bbox_pred = bbox_pred
                    bbox_pred = self.bbox_coder.decode(rois[:, 1:], bbox_pred)

                else:
                    # 即使没有self.reg_decoded_bbox也要获得一个iou_bbox_pred, 这个是用来设计新的loss的
                    iou_bbox_pred = self.bbox_coder.decode(rois[:, 1:], bbox_pred)
                ...
                # 计算loss每个featmap的loss

                # mmdet.models.losses.smooth_l1_loss.SmoothL1Loss
                # 可以直接将mmdet复制出来到mmrotate里面

                # roi的输出都是包含角度的 shape [N, 5]
                # print(pos_bbox_pred.shape, bbox_targets[pos_inds.type(torch.bool)].shape)
                losses['loss_bbox'] = self.loss_bbox(
                    pos_bbox_pred,
                    bbox_targets[pos_inds.type(torch.bool)],
                    iou_bbox_pred[pos_inds.type(torch.bool)],  # << 增加一个iou_bbox的输入
                    iou_bbox_targets[pos_inds.type(torch.bool)],  # << 增加一个iou_target的输入
